...
const Cart = (props) => {
  const visibleMenu = `opacity-100 visible absolute top-36 right-0 h-[85vh] w-1/4 flex flex-col justify-center items-center border-2 transition-all ease-in-out duration-150 ${props.theme ? 'bg-neutral-800 border-slate-100' : 'bg-slate-100 border-neutral-800'}`
  ...
  return (
    <div className={props.display ? visibleMenu :  hiddenMenu}>
      ...
      <div className='w-full h-2/6 p-4 flex flex-col justify-around items-center'>
        <ul className='w-full'>
          <li className={`border-b-2 flex justify-between text-xl py-2 ${props.theme ? 'border-slate-100' : 'border-neutral-800'}`}>Subtotal <span>$0</span></li>
